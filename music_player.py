...
def main():
    ...
    currentSong_var = StringVar()
    currentSong = ttk.Combobox(root, width=18,
                    textvariable=currentSong_var, state="readonly", height=6)
...
#     root.bind('<Double-Button-1>', double_click) #--- paly or pause song on double click
#     root.bind('<Left>', prev)
#     root.bind('<Right>', nex)
    root.bind('<Up>', v_up)
